...
class Helperfunction{
    ...
    static htmlString(){
        ...
        <div class="d-flex justify-content-center align-items-center vh-100">
            <div class="bg-aquamarine d-flex flex-column flex-md-row col-12 col-md-11 col-lg-8 px-3 py-2">

                <div id="slider-show" class="d-flex justify-content-center align-items-center col-md-7 flex-nowrap overflow-hiddens">

                </div>

                <div class="col-md-5">
                    <p id="name" class="mb-2 mx-2"></p>
                    <p id="price" class="mb-2 mx-2"></p>
...
function slider(nextIndex){
    ...
    extraImg.src = animals[currIndex].url;
    ...
    // アルゴリズム
    animalChange(currentElement, nextElement, rightOrLeft(currIndex, nextIndex));
}

// 左右を判定する関数
function rightOrLeft(currIndex, nextIndex){

    if (nextIndex >= currIndex){
        if (Math.abs(nextIndex-currIndex) >= Math.abs(currIndex+animals.length-1-nextIndex)) return "left";
        else return "right";
    }else{
        if (Math.abs(nextIndex-currIndex) >= Math.abs(nextIndex+animals.length-1-currIndex)) return "right";
        else return "left";
    }
}
...
for (let i = 0; i < animals.length; i++){
    buttons[i].addEventListener("click", function(){
        animals[i].nameChanege();
        animals[i].priceChanege();
        slider(i);
    })
}
